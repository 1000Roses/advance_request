...
func autoBroadcast(httpURL string, interval time.Duration) {
	...
	msgNum := 0
	for range ticker.C {
		req, _ := http.NewRequest("GET", httpURL+"?msg=Broadcast-"+fmt.Sprintf("%d", msgNum), nil)
...
func printStats(elapsed time.Duration) {
	...
	fmt.Printf("Errors:               %d\n", errCount)
	fmt.Printf("Duration:             %v\n", elapsed)
	fmt.Printf("Send RPS:             %.2f\n", rps)
	fmt.Printf("Throughput received:  %d msg/s\n", int(float64(received)/elapsed.Seconds()))
}